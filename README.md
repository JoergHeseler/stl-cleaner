...
Incorrect STL files can disrupt 3D workflows and archiving. Some popular 3D processing tools do not export 3D data to STL format strictly according to the [Marshall Burns specification](https://www.fabbers.com/tech/STL_Format). For example, MeshLab writes the generator name after the `solid` tag and its processing librarys' name after the `endsolid` tag. Furthermore, vertices can also take on negative values, which is not in accordance with the specification. Therefore, the STL Cleaner tool was developed as part of this project to correct these deviations without changing the geometry:
